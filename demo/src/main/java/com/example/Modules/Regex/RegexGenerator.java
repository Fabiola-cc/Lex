...
public class RegexGenerator {
    ...
    public static List<RegexToken> generateCombinedRegex(Map<String, String> processedMap) {
        List<RegexToken> combinedTokens = new ArrayList<>();

        for (Map.Entry<String, String> entry : processedMap.entrySet()) {
            String regex = entry.getKey();
            String token = entry.getValue();

            combinedTokens.add(new RegexToken("(", true));
            combinedTokens.add(new RegexToken("(", true));

            List<RegexToken> regexTokens = convertToArray(regex);

            combinedTokens.addAll(regexTokens);

            combinedTokens.add(new RegexToken(")", true));

            RegexToken tokenToken = new RegexToken(token, false);
            tokenToken.setIsToken(true);
            combinedTokens.add(tokenToken);
            combinedTokens.add(new RegexToken(")", true));
            combinedTokens.add(new RegexToken("|", true));
        }

        combinedTokens.remove(combinedTokens.size() - 1);
