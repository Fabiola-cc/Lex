...
public class Calculate_tree {
  ...
  /*
   * Crear una función que toma un List<RegexToken> que nos da la información de
   * si es un operador o no y que valor tiene
   * Tenemos que regresar una lista de nodos en donde se indique la jerarquia
   * 
   * Proceso para lograrlo: leer primer objeto de Lista input si no es operdor
   * obtener valor y grardarlo en un stack
   * , lo mismo con lo siguinte hasta que se tope con un operador va a crear un
   * nodo operador
   */
   ...
   * Retorna la lista completa de nodos con sus relaciones jerárquicas
   * establecidas.
   */
  ...
  public List<node> convertPostfixToTree(List<RegexToken> postfix) {
    ...
    newRoot.getNodes().add(treeNodes.size() - 2);
    newRoot.getNodes().add(treeNodes.size() - 1);
   ...
   * 4. Si stack no esta vacio, el primero que fue el ultimo en ingresar sera
